...
class MPC(Controller):
    ...
    def __init__(self, params, calibrate=False):
        ...
        print("Created an MPC controller, prop mode %s, %d particles. Calibration set to %s" % (self.prop_mode, self.npart, self.should_calibrate) +
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs, logdir=None):
        ...
        # if self.should_calibrate and logdir:
            # self.model.plot_calibration(new_train_in[0], new_train_targs[0], logdir)
...
#
#        if self.should_calibrate:
#            self.model.calibrate(self.train_in, self.train_targs, **self.model_train_cfg)
    ...
    def act(self, obs, t, get_pred_cost=False):
        ...
        if get_pred_cost and not (self.log_traj_preds or self.log_particles):
            ...
            if self.log_particles:
                ...
                self.pred_means.append(curr_mean.squeeze()) #np.mean(pred_traj, axis=1))
    ...
    def dump_logs(self, primary_logdir, iter_logdir):
        ...
        if self.log_particles:
            if iter_logdir:
                savemat(os.path.join(iter_logdir, "predictions.mat"), {"predictions": self.pred_particles, "costs": self.pred_costs})
            self.pred_particles = []
            self.pred_costs = []
        elif self.log_traj_preds:
            if iter_logdir:
                savemat(
                    ...
                    {"means": self.pred_means, "vars": self.pred_vars, "costs":  self.pred_costs}
                )
            self.pred_means, self.pred_vars = [], []
            self.pred_costs = []
